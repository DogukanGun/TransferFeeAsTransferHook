...
import { TransferHookWhitelist } from "../target/types/transfer_hook_whitelist";
import { BN } from 'bn.js';

describe("transfer_hook_whitelist", () => {
  // Configure the client to use the local cluster.
  const provider = anchor.AnchorProvider.env();
  anchor.setProvider(provider);

  const program = anchor.workspace.TransferHookWhitelist as Program<TransferHookWhitelist>;
  ...
  it('Create WSol Token Accounts', async () => {
  ...
  it('Set Transfer Fee', async () => {
    const setTransferFee = await program.methods.setFee(new BN(1)).accounts({ signer: wallet.publicKey }).instruction();
    const transaction = new Transaction().add(setTransferFee);
    const txSig = await sendAndConfirmTransaction(connection, transaction, [wallet.payer], { skipPreflight: true });
  ...
  it('Remove account from white list', async () => {
    const removeAccountFromWhiteListInstruction = await program.methods
      .removeFromWhitelist()
      .accounts({
        removedAccount: destinationTokenAccount,
        signer: wallet.publicKey,
      })
      .instruction();

    const transaction = new Transaction().add(removeAccountFromWhiteListInstruction);
  ...
  it('Transfer Hook with Extra Account Meta', async () => {
    ...
    assert.equal(Number(tokenAccount.amount), amount+(1 * 10 ** decimals));
  });
